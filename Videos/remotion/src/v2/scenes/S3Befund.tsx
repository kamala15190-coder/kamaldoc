...
/** SCENE 3 — Befundassistent: medical document transforms into clean summary */
export const S3Befund: React.FC = () => {
  ...
  // Before-card: blurred medical text. Morphs to After-card at ~frame 90
  ...
  return (
    <SceneShell glowY={46}>
      ...
      <div style={{
        ...
        <IconOrb size={130} tint={V2.primaryLight}>
          <StethoscopeIcon size={66} />
        </IconOrb>
      </div>
      ...
      <div style={{ position: 'absolute', left: '50%', top: 1090, transform: 'translateX(-50%)', width: 820, height: 500 }}>
        ...
        <div style={{
          ...
          <div style={{ fontFamily: V2.font, fontSize: 18, color: V2.textMuted, marginBottom: 10 }}>Laborbefund · ID 2024-0812</div>
        ...
        {/* AFTER — clean glass card with verdict */}
        <div style={{
          position: 'absolute', inset: 0,
          borderRadius: 18,
          background: V2.glassStrong,
      ...
      <div style={{
        ...
        {langs.map((code) => (
          <div key={code} style={{
            ...
            fontFamily: V2.font, fontSize: 26, fontWeight: 700, color: V2.text,
            letterSpacing: 1,
          }}>{code}</div>
        ))}
        <div style={{
          padding: '10px 18px', borderRadius: 10,
          background: 'rgba(99, 89, 255, 0.18)',
          border: `1px solid ${V2.primary}`,
          fontFamily: V2.font, fontSize: 26, fontWeight: 600, color: V2.primaryLight,
          letterSpacing: 0.3,
        }}>+ 50 Sprachen</div>
      </div>
    </SceneShell>
