...
export default function RegisterPage() {
  ...
  const handleAppleSignUp = useAppleSignIn({ setError, setLoading })
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div className="glass-card animate-fade-in-up" style={{ padding: 24 }}>
          ...
          {/* Apple Button — iOS only (Guideline 4.8). Black per Apple's HIG. */}
          {isIos() && (
            <button onClick={handleAppleSignUp} disabled={loading || !canSubmit} style={{
              width: '100%', padding: '12px 0', marginTop: 10, fontSize: 15, fontWeight: 600,
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              background: '#000', color: '#fff', border: 'none', borderRadius: 12,
              cursor: 'pointer', opacity: (loading || !canSubmit) ? 0.5 : 1,
            }}>
              <svg style={{ width: 17, height: 17, marginTop: -2 }} viewBox="0 0 384 512" fill="#fff" aria-hidden="true">
