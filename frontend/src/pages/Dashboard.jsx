    ...
    stats: (
      <div style={{ animation: 'scaleIn 0.4s ease both', animationDelay: '0.05s' }}>
        ...
        <div data-intro="phishing" onClick={() => navigate('/phishing')} className="tile-hover"
          ...
          <div style={{ width: 34, height: 34, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(232,154,82,0.18)', flexShrink: 0 }}>
            <ShieldAlert style={{ width: 18, height: 18, color: 'var(--amber)' }} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: tc.text }}>{t('phishing.title', { defaultValue: 'Phishing-Prüfung' })}</div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{t('phishing.dashboardHint', { defaultValue: 'Verdächtige Nachricht prüfen' })}</div>
          </div>
