...
const Doka = lazy(() => import('./pages/Doka'));
...
const TAB_ITEMS = [
  ...
  { path: '/doka', labelKey: 'nav.doka', icon: MessageCircle },
          ...
          if (isCenter) {
            return (
              <Link key={path} to={path} className="plus-button" onClick={() => { if (Capacitor.isNativePlatform()) { Haptics.impact({ style: ImpactStyle.Light }).catch(() => {}); } }} style={{
                ...
                boxShadow: '0 4px 20px rgba(232,154,82, 0.35)',
                textDecoration: 'none',
...
function PWAInstallBanner() {
  ...
  return (
    <div style={{
      ...
      padding: '12px 16px',
      background: 'linear-gradient(135deg, rgba(232,154,82,0.95), rgba(232,154,82,0.95))',
      backdropFilter: 'blur(12px)',
      display: 'flex', alignItems: 'center', gap: 12,
      animation: dismissed ? 'slideUp 0.3s ease forwards' : 'slideDown 0.4s cubic-bezier(0.16,1,0.3,1)',
      boxShadow: '0 4px 24px rgba(232,154,82,0.3)',
    }}>
      <div style={{
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)' }}>
      ...
      <main style={!isAuthPage ? {
        ...
        <Routes>
          ...
          <Route path="/doka" element={<PrivateRoute><Doka /></PrivateRoute>} />
