...
const STEPS = [
  ...
  { route: '/doka',     target: '[data-intro="doka"]',       titleKey: 'intro.step3Title',      descKey: 'intro.step3Desc' },
  { route: '/phishing', target: '[data-intro="phishing"]',   titleKey: 'intro.phishingTitle',   descKey: 'intro.phishingDesc' },
  { route: '/email',    target: '[data-intro="email"]',      titleKey: 'intro.emailTitle',      descKey: 'intro.emailDesc' },
  { route: '/',         target: '[data-intro="search"]',     titleKey: 'intro.searchTitle',     descKey: 'intro.searchDesc' },
...
export default function IntroGuide() {
    ...
    return (
      <div style={{
        ...
        <div style={{ position: 'relative' }}>
          {Array.from({ length: 40 }).map((_, i) => (
            <div key={i} style={{
              ...
              background: ['#E89A52', '#FF5F6D', '#FFD700', '#10B981', '#60A5FA', '#F472B6', '#FBBF24'][i % 7],
              top: '50%', left: '50%',
        ...
        <button onClick={() => { setShow(false); setConfetti(false); navigate('/'); }} style={{
          ...
          background: 'linear-gradient(135deg, #E89A52, #B66B2C)',
          color: 'white', border: 'none', fontSize: 16, fontWeight: 700,
          cursor: 'pointer', boxShadow: '0 4px 20px rgba(232,154,82,0.4)',
        }}>
          {t('intro.letsGo')}
        </button>
        <style>{`
  ...
  let tooltipStyle = {
    ...
    border: '1px solid rgba(232,154,82,0.3)',
    ...
    <>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 9999, pointerEvents: 'auto' }}>
        ...
        {targetRect && (
          <div style={{
            ...
            borderRadius: 14,
            border: '2px solid rgba(232,154,82,0.8)',
            boxShadow: '0 0 0 4px rgba(232,154,82,0.2), 0 0 20px rgba(232,154,82,0.3)',
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {STEPS.map((_, i) => (
              <div key={i} style={{
                ...
                background: i === step ? '#E89A52' : i < step ? 'rgba(232,154,82,0.5)' : 'rgba(255,255,255,0.15)',
            ...
            {currentStep.interactive === 'absender' ? (
              <>
                ...
                <button onClick={handleSaveSender} disabled={savingSender || senderSaved} style={{
                  ...
                  borderRadius: 10, background: senderSaved ? '#10B981' : 'linear-gradient(135deg, #E89A52, #B66B2C)',
                  border: 'none', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer',
                  boxShadow: '0 2px 12px rgba(232,154,82,0.3)',
              ...
              <button onClick={handleNext} style={{
                display: 'flex', alignItems: 'center', gap: 4, padding: '8px 18px',
                borderRadius: 10, background: 'linear-gradient(135deg, #E89A52, #B66B2C)',
                border: 'none', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer',
                boxShadow: '0 2px 12px rgba(232,154,82,0.3)',
              }}>
      ...
      <style>{`
        @keyframes introPulse {
          0%, 100% { box-shadow: 0 0 0 4px rgba(232,154,82,0.2), 0 0 20px rgba(232,154,82,0.3); }
          50% { box-shadow: 0 0 0 8px rgba(232,154,82,0.1), 0 0 30px rgba(232,154,82,0.5); }
        }
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isActive ? 'rgba(232,154,82,0.25)' : 'rgba(255,255,255,0.06)',
                border: isActive ? '1px solid rgba(232,154,82,0.6)' : '1px solid rgba(255,255,255,0.08)',
                transition: 'all 0.15s ease',
              }}
              title={lang.label}
            >
              <span style={{ fontSize: 18, lineHeight: 1 }}>{lang.flag}</span>
              <span style={{ fontSize: 9, color: isActive ? '#F6C58A' : 'var(--text-muted, #888)', fontWeight: isActive ? 700 : 400 }}>
    ...
    <div style={{ display: 'flex', gap: 10, marginBottom: 8 }}>
      ...
      <button
        ...
        style={{
          flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
          padding: '10px 0', borderRadius: 10, cursor: 'pointer',
          background: theme === 'dark' ? 'rgba(232,154,82,0.2)' : 'rgba(255,255,255,0.06)',
          border: theme === 'dark' ? '2px solid rgba(232,154,82,0.6)' : '1px solid rgba(255,255,255,0.1)',
          color: theme === 'dark' ? '#F6C58A' : 'var(--text-secondary, #aaa)',
          fontWeight: 700, fontSize: 13, transition: 'all 0.2s ease',
        }}
      >
