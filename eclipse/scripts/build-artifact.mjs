...
const SEQS = {
  ...
  macro:    { count: 120, step: 2, w: 1280, q: 0.60 },
  exploded: { count: 120, step: 2, w: 1280, q: 0.65 },
};
...
const atmosphere = await recode(`http://127.0.0.1:${port}/assets/stills/atmosphere.webp`, 1600, 0.72);
await browser.close();
server.close();
...
css = [
  fontCss('Cormorant Garamond', 500, 'normal', 'cormorant-garamond-latin-500-normal.woff2'),
  fontCss('Cormorant Garamond', 600, 'normal', 'cormorant-garamond-latin-600-normal.woff2'),
  fontCss('Cormorant Garamond', 500, 'italic', 'cormorant-garamond-latin-500-italic.woff2'),
  fontCss('Manrope', 300, 'normal', 'manrope-latin-300-normal.woff2'),
  fontCss('Manrope', 500, 'normal', 'manrope-latin-500-normal.woff2'),
  css.replace("url('../assets/stills/atmosphere.webp')", `url(${atmosphere})`),
].join('\n');
...
js = js.replace("new Scrubber(document.getElementById('macroCanvas'), 'macro', 120)",
  "new Scrubber(document.getElementById('macroCanvas'), 'macro', window.__FRAMES.macro.length)");
js = js.replace("new Scrubber(document.getElementById('explodedCanvas'), 'exploded', 120)",
  "new Scrubber(document.getElementById('explodedCanvas'), 'exploded', window.__FRAMES.exploded.length)");
...
const outFile = path.join(DIST, 'eclipse-artifact.html');
