...
async def init_db():
    ...
    try:
        await db.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS doka_conversations (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id TEXT NOT NULL,
                title TEXT,
                created_at TEXT NOT NULL DEFAULT (datetime('now','localtime')),
            ...
            CREATE TABLE IF NOT EXISTS doka_messages (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ...
                created_at TEXT NOT NULL DEFAULT (datetime('now','localtime')),
                FOREIGN KEY (conversation_id) REFERENCES doka_conversations(id) ON DELETE CASCADE
            );

            -- OAuth-State-Store (Phase H2) — CSRF-Schutz mit TTL statt Prozess-Memory
...
VERSIONED_MIGRATIONS: list[tuple[int, str, list[str]]] = [
    ...
    (
        2,
        "Indices für Doka-Chat (Phase D)",
        [
            "CREATE INDEX IF NOT EXISTS idx_doka_conversations_user ON doka_conversations(user_id, updated_at)",
            "CREATE INDEX IF NOT EXISTS idx_doka_messages_conversation ON doka_messages(conversation_id, id)",
        ],
    ),
    (
        3,
        "Index für Phishing-Checks (Phase E)",
        [
            "CREATE INDEX IF NOT EXISTS idx_phishing_checks_user ON phishing_checks(user_id, created_at)",
        ],
    ),
    (
        4,
        "Index für Connector-Accounts (Phase F)",
        [
            "CREATE INDEX IF NOT EXISTS idx_connector_accounts_user ON connector_accounts(user_id, connector_type)",
        ],
    ),
]
