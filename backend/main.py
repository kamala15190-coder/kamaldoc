...
from subscription import (
    ...
    check_connector_limit,
    check_doka_limit,
    check_phishing_limit,
...
# H7: Dev-Origins NUR bei striktem APP_ENV == 'development'. Jeder andere Wert
# (production, staging, Tippfehler, leer→default 'development') bekommt keine
# Dev-Origins außer dem echten Entwicklungsmodus.
_CORS_ORIGINS = _PROD_ORIGINS + (_DEV_ORIGINS if _APP_ENV == "development" else [])
...
# --- Per-User Rate Limiting (H4) ---
# IP-Limits oben greifen nicht bei NAT/Shared-IP. Diese In-Memory-Buckets
# begrenzen teure Aktionen zusätzlich pro authentifiziertem User.
_user_rate_buckets: dict[str, list] = defaultdict(list)


def enforce_user_rate(user_id: str, key: str, limit: int, window: int = 60):
    now = time.time()
    bucket_key = f"{user_id}:{key}"
    bucket = _user_rate_buckets[bucket_key]
    bucket[:] = [t for t in bucket if now - t < window]
    if len(bucket) >= limit:
        raise HTTPException(
            status_code=429,
            detail={"code": "RATE_LIMIT", "message": f"Zu viele Anfragen für {key}. Bitte kurz warten."},
        )
    bucket.append(now)
...
async def startup():
    # H3: Wenn Stripe aktiv ist, MUSS ein gültiges Webhook-Secret gesetzt sein —
    # sonst Boot-Fehler statt stillem Ignorieren gefälschter Webhooks.
    from subscription import STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET
    if STRIPE_SECRET_KEY and (not STRIPE_WEBHOOK_SECRET or STRIPE_WEBHOOK_SECRET == "whsec_PLACEHOLDER"):
        raise RuntimeError(
            "STRIPE_SECRET_KEY ist gesetzt, aber STRIPE_WEBHOOK_SECRET fehlt oder ist ein Platzhalter. "
            "Setze ein gültiges Webhook-Secret oder deaktiviere Stripe (STRIPE_SECRET_KEY leer)."
        )
...
# and process restarts; expired states are purged on each write/read.
_OAUTH_STATE_TTL = 600


async def _store_oauth_state(token: str, platform: str):
    from datetime import timedelta
    now = datetime.now()
    db = await get_db()
    try:
        await db.execute("DELETE FROM oauth_states WHERE expires_at < ?", (now.isoformat(),))
        await db.execute(
            "INSERT OR REPLACE INTO oauth_states (state, payload, expires_at) VALUES (?, ?, ?)",
            (token, platform, (now + timedelta(seconds=_OAUTH_STATE_TTL)).isoformat()),
        )
        await db.commit()
    finally:
        await db.close()
...
async def _pop_oauth_state(token: str) -> bool:
    """Return True if the state exists and is unexpired; consume it either way."""
    now = datetime.now()
    db = await get_db()
    try:
        cursor = await db.execute(
            "SELECT expires_at FROM oauth_states WHERE state = ?", (token,)
        )
...
async def google_login(platform: str = "web"):
    ...
    await _store_oauth_state(token, platform)
    params = {
...
async def google_callback(code: str = None, state: str = "", error: str = None):
    ...
    if ":" in state:
        ...
        state_token = parts[1]
        if not await _pop_oauth_state(state_token):
...
async def upload_document(
    ...
    enforce_user_rate(user_id, "upload", 30, 60)
...
class DokaConversationCreate(BaseModel):
    title: str | None = None


def _doka_message_to_dict(row) -> dict:
    """Serialize a doka_messages row, decoding the JSON columns."""
    import json

    msg = dict(row)
    for field in ("attachments", "tool_calls"):
        raw = msg.get(field)
        if raw:
            try:
...
async def _doka_get_conversation(db, conv_id: int, user_id: str):
    """Fetch a conversation owned by user_id or raise 404."""
    cursor = await db.execute(
        "SELECT * FROM doka_conversations WHERE id = ? AND user_id = ?", (conv_id, user_id)
    )
    row = await cursor.fetchone()
    if not row:
...
@app.post("/api/doka/conversations")
async def create_doka_conversation(
    payload: DokaConversationCreate, user_id: str = Depends(get_current_user)
):
    db = await get_db()
    try:
        cursor = await db.execute(
            "INSERT INTO doka_conversations (user_id, title) VALUES (?, ?)",
            (user_id, (payload.title or None)),
        )
        await db.commit()
        conv_id = cursor.lastrowid
        row = await _doka_get_conversation(db, conv_id, user_id)
        return dict(row)
    finally:
        await db.close()


@app.get("/api/doka/conversations")
async def list_doka_conversations(user_id: str = Depends(get_current_user)):
    db = await get_db()
    try:
        cursor = await db.execute(
            "SELECT * FROM doka_conversations WHERE user_id = ? ORDER BY updated_at DESC, id DESC",
            (user_id,),
        )
        rows = await cursor.fetchall()
...
@app.get("/api/doka/conversations/{conv_id}")
async def get_doka_conversation(conv_id: int, user_id: str = Depends(get_current_user)):
    db = await get_db()
    try:
        conv = await _doka_get_conversation(db, conv_id, user_id)
        cursor = await db.execute(
            "SELECT * FROM doka_messages WHERE conversation_id = ? ORDER BY id ASC", (conv_id,)
        )
        rows = await cursor.fetchall()
        return {**dict(conv), "messages": [_doka_message_to_dict(r) for r in rows]}
    finally:
        await db.close()
...
DOKA_ATTACHMENTS_DIR = DATA_DIR / "doka_attachments"


async def _doka_extract_attachment_text(upload: UploadFile) -> tuple[dict | None, str]:
    """Persist an attached file and OCR it. Returns (attachment_meta, extracted_text)."""
    os.makedirs(DOKA_ATTACHMENTS_DIR, exist_ok=True)
    ...
    meta = {"filename": upload.filename, "stored": safe_name, "type": ext.lstrip(".")}
    return meta, (text or "")


@app.post("/api/doka/conversations/{conv_id}/message")
async def doka_send_message(
    conv_id: int,
    message: str = Form(""),
    lawyer_mode: bool = Form(False),
    language: str = Form("de"),
    file: UploadFile = File(None),
    user_id: str = Depends(get_current_user),
):
    """Append a user message and stream Doka's response (SSE)."""
    import json

    enforce_user_rate(user_id, "doka", 30, 60)
    await check_doka_limit(user_id)

    attachment_meta, attachment_text = (None, "")
    if file is not None and (file.filename or ""):
        attachment_meta, attachment_text = await _doka_extract_attachment_text(file)

    if not (message or "").strip() and not attachment_text:
        raise HTTPException(400, "Leere Nachricht")

    # Persist the user turn and assemble the history for the model.
    db = await get_db()
    try:
        conv = await _doka_get_conversation(db, conv_id, user_id)
        await db.execute(
            "INSERT INTO doka_messages (conversation_id, role, content, attachments) VALUES (?, ?, ?, ?)",
            (conv_id, "user", message, json.dumps(attachment_meta, ensure_ascii=False) if attachment_meta else None),
        )
        if not conv["title"]:
            title = (message.strip()[:60] or (attachment_meta or {}).get("filename") or "Neue Unterhaltung")
            await db.execute(
                "UPDATE doka_conversations SET title = ?, updated_at = datetime('now','localtime') WHERE id = ?",
                (title, conv_id),
            )
        await db.commit()

        cursor = await db.execute(
            "SELECT role, content FROM doka_messages WHERE conversation_id = ? ORDER BY id ASC", (conv_id,)
        )
...
PHISHING_DIR = DATA_DIR / "phishing_uploads"
...
async def phishing_check(
    ...
    """Prüft Text, ein hochgeladenes Bild/PDF oder ein bestehendes Dokument auf Phishing."""
    import json

    enforce_user_rate(user_id, "phishing", 20, 60)
    await check_phishing_limit(user_id)
    ...
    if document_id is not None:
        ...
        try:
            cursor = await db.execute(
                "SELECT volltext, zusammenfassung FROM documents WHERE id = ? AND user_id = ?",
                (document_id, user_id),
            )
            row = await cursor.fetchone()
        finally:
        ...
        if not row:
            raise HTTPException(404, "Dokument nicht gefunden")
        content = (row["volltext"] or row["zusammenfassung"] or "").strip()
        used_doc_id = document_id

    elif file is not None and (file.filename or ""):
        ext = Path(file.filename or "").suffix.lower()
        if ext not in ALLOWED_EXTENSIONS:
            raise HTTPException(400, "Dateityp nicht unterstützt (jpg, png, pdf)")
        os.makedirs(PHISHING_DIR, exist_ok=True)
        raw = await file.read()
        tmp_path = PHISHING_DIR / f"{uuid.uuid4().hex}{ext}"
    ...
    if not content:
        raise HTTPException(400, "Kein Text zum Prüfen gefunden")

    result = await check_phishing(content, language)

    db = await get_db()
    try:
        await db.execute(
            "INSERT INTO phishing_checks (user_id, document_id, risk_score, verdict, reasoning) VALUES (?, ?, ?, ?, ?)",
            (user_id, used_doc_id, result["risk_score"], result["verdict"],
             json.dumps(result["reasoning"], ensure_ascii=False)),
        )
        await db.commit()
    finally:
        await db.close()

    await increment_usage(user_id, "phishing_checks_month")
    return result
...
async def connectors_accounts(user_id: str = Depends(get_current_user)):
    from connectors_service import list_accounts
    return {"accounts": await list_accounts(user_id)}


@app.post("/api/connectors/imap/connect")
async def connectors_imap_connect(payload: ImapConnectRequest, user_id: str = Depends(get_current_user)):
    from connectors_service import add_imap_account
    from crypto import encryption_available
    ...
    try:
        account_id = await add_imap_account(
            user_id, payload.connector_type, payload.display_name,
            payload.email, payload.password, payload.host, payload.port,
        )
...
async def connectors_patch(account_id: int, patch: ConnectorPatch, user_id: str = Depends(get_current_user)):
    db = await get_db()
    try:
        cursor = await db.execute(
            "SELECT id FROM connector_accounts WHERE id = ? AND user_id = ?", (account_id, user_id)
        )
        if not await cursor.fetchone():
            raise HTTPException(404, "Konto nicht gefunden")
        sets, params = [], []
        if patch.display_name is not None:
            sets.append("display_name = ?"); params.append(patch.display_name)
        if patch.status is not None:
            sets.append("status = ?"); params.append(patch.status)
        if sets:
...
async def connectors_delete(account_id: int, user_id: str = Depends(get_current_user)):
    from connectors_service import delete_account
    ok = await delete_account(user_id, account_id)
    if not ok:
        raise HTTPException(404, "Konto nicht gefunden")
    return {"status": "ok"}


@app.post("/api/connectors/accounts/{account_id}/sync")
async def connectors_sync(account_id: int, user_id: str = Depends(get_current_user)):
    from connectors_service import sync_account
    enforce_user_rate(user_id, "connector_sync", 20, 60)
    return await sync_account(user_id, account_id)
